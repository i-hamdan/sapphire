...
def process_map():
    ...
    for cnt in cnts_h:
        ...
        full_map_data.append({"type": "highway", "label": "6 Lane Highway", "points": cnt.reshape(-1, 2).tolist(), "center": [cX, cY]})
        # Annotation: Bold Red
        cv2.drawContours(annotated_img, [cnt], -1, (0, 0, 255), 8) # Thick Red
    ...
    for cnt in cnts_r:
        ...
        road_count += 1
        full_map_data.append({"type": "road", "label": f"Arterial Road {road_count}", "points": cnt.reshape(-1, 2).tolist(), "center": [cX, cY]})
    ...
    # Resorts & Green
    for cat, color, range_hsv in [
        ("green", (0, 150, 0), ([35, 50, 40], [90, 255, 255])),
        ("resort", (150, 0, 150), ([135, 30, 50], [170, 255, 255]))
    ]:
        m = cv2.inRange(hsv, np.array(range_hsv[0]), np.array(range_hsv[1]))
        m = cv2.morphologyEx(m, cv2.MORPH_CLOSE, np.ones((10,10), np.uint8))
        cs, _ = cv2.findContours(m, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        for i, cnt in enumerate(cs):
            area = cv2.contourArea(cnt)
            if area < 2000: continue
            M = cv2.moments(cnt)
            cX, cY = (int(M["m10"]/M["m00"]), int(M["m01"]/M["m00"])) if M["m00"] != 0 else (0,0)
            label = cat.capitalize()
            full_map_data.append({"type": cat, "label": label, "points": cnt.reshape(-1, 2).tolist(), "center": [cX, cY]})
    ...
    print(f"Extraction Successful. Found {len(processed_plots)} plots, {road_count} road segments, and highway={highway_found}.")
